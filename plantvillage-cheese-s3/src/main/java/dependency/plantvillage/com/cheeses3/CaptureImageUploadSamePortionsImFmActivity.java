...
public class CaptureImageUploadSamePortionsImFmActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_capture_image_upload_same_portions_im_fm);
        ...
        System.out.println("\n\n\n\nFilename in Capture IMFM SAME PORTIONS = " +filename);
        portion_food_id_list = new ArrayList<String>(food_list.size()); // Make a new list



        int food_list_length = food_list.size();
        for ( int n = 0; n < portion_id_list.size(); n++) { // Loop through every name/phone number combo
          // System.out.println(portion_id_list.get(n));
        }
        if (food_list_length != portion_list.size()) { // Too many names, or too many numbers

        }
        final ArrayList<String> food_and_portion_list = new ArrayList<String>(food_list_length); // Make a new list
        ...
        btn_change_food_keep_portion.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //Toast.makeText(CaptureImageUploadSamePortionsImFmActivity.this, "Edit recipe list has not been implemented yet.", Toast.LENGTH_SHORT).show();
                editRecipe(view);
            }
        });
            ...
            public void onItemClick(AdapterView<?> parent, final View view,
                ...
                view.animate().setDuration(2000).alpha(0)
                        .withEndAction(new Runnable() {
                            @Override
                            public void run() {
                                //food_and_portion_list.remove(item);
                                String[] currencies = item.split(" ");
    ...
    public void editRecipe(View view) {
        ...
        intent.putExtra("portion_size", portion_size);
        intent.putExtra("portion_id", portionID);
        intent.putExtra("FROM_ACTIVITY", "CAPTURE_SAME_PORTIONS");
        intent.putExtra("dishType",dishType);
        intent.putExtra("recipeCodeList", recipeCodeList);
        intent.putExtra("filename",filename);
    ...
    private void processImageByPath(String path){
        ...
        transferUtility = new TransferUtility(s3, CaptureImageUploadSamePortionsImFmActivity.this);
        ...
        if (!file.exists()) {
            Toast.makeText(CaptureImageUploadSamePortionsImFmActivity.this, "File Not Found!", Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == CAMERA_REQUEST) {
            ...
            try {
                ...
                Toast.makeText(CaptureImageUploadSamePortionsImFmActivity.this, "Image Saved", Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
    ...
    private void storeImageTosdCard(Bitmap processedBitmap) {
        try {
            ...
            Toast.makeText(CaptureImageUploadSamePortionsImFmActivity.this, "Saving file to DCIM/Cheese/"+name,Toast.LENGTH_LONG).show();
    ...
    static class StableArrayAdapter extends ArrayAdapter<String> {
